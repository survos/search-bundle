...
final class SearchIndexCommand
{
    ...
    public function __invoke(
        ...
        foreach ($descriptors as $descriptor) {
            ...
            // External engines own their own index lifecycle in their own bundle
            // (survos/elastic-bundle, survos/meili-bundle). This command is only for the
            // database-native indexes, which need no client library and no extra service.
            if (isset($parameters['mappings']) || isset($parameters['index'])) {
                $io->note(sprintf(
                    'Skipping "%s": externally indexed. Use elastic:index:create / elastic:index:populate.',
                    $descriptor->code,
                ));
                continue;
            }
